...
# Make `app` importable when alembic is invoked from backend/ (its cwd).
...
# Always migrate the same database the app itself connects to
# (HR_DATABASE_URL) — never a stale URL hardcoded in alembic.ini.
