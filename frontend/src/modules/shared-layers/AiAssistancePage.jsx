        ...
        <div className="lg:col-span-2 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col h-[520px] justify-between">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <Bot className="h-5 w-5 text-[#3B82F6]" /> Copilot Assistant
            </h3>
            ...
            {messages.map((m, idx) => (
              ...
              >
                <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl ${
                  m.role === "user" ? "bg-[#3B82F6] text-white" : "bg-slate-100 text-[#3B82F6]"
                }`}>
                  {m.role === "user" ? "U" : <Sparkles className="h-4.5 w-4.5" />}
                </div>
                <div className={`p-3.5 rounded-2xl ${
                  m.role === "user" ? "bg-[#3B82F6] text-white rounded-tr-none" : "bg-slate-50 border border-slate-100 text-slate-800 rounded-tl-none"
                }`}>
          ...
          <form onSubmit={handleSend} className="relative">
            <input 
              ...
              className="w-full rounded-full border border-slate-200 bg-slate-50 py-3.5 pl-5 pr-14 text-sm text-slate-800 placeholder-slate-450 outline-none focus:bg-white focus:border-[#3B82F6]"
            />
            <button 
              type="submit" 
              className="absolute right-2 top-1/2 -translate-y-1/2 flex h-9 w-9 items-center justify-center rounded-full bg-[#3B82F6] hover:bg-[#2563EB] text-white shadow-sm transition"
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between h-[520px]">
          <div>
            <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Settings className="h-5 w-5 text-[#3B82F6]" /> Model Configuration
            </h3>
          ...
          <div className="p-4 rounded-2xl bg-[#3B82F6]/5 border border-[#3B82F6]/10 flex gap-2.5 items-start">
            <Zap className="h-5 w-5 text-[#3B82F6] flex-shrink-0 mt-0.5" />
