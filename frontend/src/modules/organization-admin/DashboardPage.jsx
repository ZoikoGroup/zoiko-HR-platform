...
import { DashboardSkeleton } from "../../components/OrgAdminSkeleton";
...
export default function OrgAdminDashboardPage() {
  ...
  const renderKpi = (kpi) => {
    const val = stats ? fmt(stats[kpi.key]) : "—";
  ...
  return (
    <div className="font-['Inter',system-ui,sans-serif] -m-4 sm:-m-6 lg:-m-8 p-4 sm:p-6 lg:p-8" style={{ background: "#F0F4F8", color: INK, minHeight: "calc(100vh - 4rem)" }}>
      ...
      <Suspense fallback={<div className="mt-8 text-center text-[13px]" style={{ color: INK_SOFT }}>Loading charts...</div>}>
        <DashboardCharts stats={stats} loading={false} totalEmployees={totalEmployees} departments={departments} activeEmployees={activeEmployees} />
      </Suspense>
    </div>
  );
}
